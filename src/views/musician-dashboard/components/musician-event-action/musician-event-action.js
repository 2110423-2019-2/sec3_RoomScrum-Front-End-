...
const MusicianEventAction = ({application, refreshCallback, debug}) => {
    ...
    const canCancel = debug || !(eventStatus == EventStatus.COMPLETE || eventStatus == EventStatus.CANCELLED);
    ...
    return (
        <div className="musician-event-actions">
            { (canAcceptPayment) && <AcceptPaymentButton eventId={eventId} onSuccess={refreshCallback}/> }
            { canCancel && <CancelEventButton eventId={eventId}  onSuccess={refreshCallback}/> }
            { canCompleteEvent && <CompleteEventButton eventId={eventId} onSuccess={refreshCallback}/>}
            {
                <PayByQRButton
            ...
            {
                canApply && <ApplyEventButton eventId={eventId} onSuccess={refreshCallback}/>
            }
        </div>
    )
};
