...
export default ({
  ...
  const requestAcceptMusician = userId => () => {
    request
      .post(`${config.API_URL}/admin/user/approve`)
      .send({ userId })
      .then(() => {
        onAcceptSuccess();
        onRequestClose();
  ...
  const requestRejectMusician = userId => () => {
    request
      .post(`${config.API_URL}/admin/user/reject`)
      .send({ userId })
      .then(() => {
        onRejectSuccess();
  ...
  return (
    <>
      <Modal
        className="musician-info-dialog shadow rounded-lg"
        isOpen={isOpen}
      ...
      >
        ...
        <div className="dialog-content row justify-content-center m-0">
          <div className="center">
            <img
              src={profileImage}
              className="profile-image btn img-btn"
              onClick={() => {
          ...
          <div className="col-12 col-sm-10 mt-4 info">
            <dl className="row">
              ...
              <Field
                fieldName="Age"
                value={calcAge(new Date(), new Date(birthdate))}
              />
              ...
              <Field
                fieldName="Video"
                value={<a href={videoUrl}> {videoUrl}</a>}
              />
              <Field
                fieldName="Nation Card Image"
                value={
                  <img
                    src={nationalCardImage}
                    className="national-id-image btn img-btn"
                    onClick={() => {
          ...
          <div className="col-12">
            <div className="row justify-content-between m-3">
              <button
                onClick={() => acceptMusician(userId)}
                className="btn btn-success col-6 rounded-0"
              >
                {" "}
                Accept{" "}
              </button>
              <button
                onClick={() => rejectMusician(userId)}
